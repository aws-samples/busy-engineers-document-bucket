...
program
  ...
  .action(async (path, cmdObj) => {
    assert(!path !== process.stdin.isTTY, 'Source must be provided. Either stdin or a path')
...
program
  ...
  .option('-c, --expected-context <name:value>', 'Encryption context pairs to verify', pairs, {})
  .option('-k, --expected-context-keys <name>', 'Encryption context keys to verify', collect, [])
  .action((key, path, cmdObj) => {
    ...
    index
      .retrieve(key,  { expectedContext,  expectedContextKeys })
      .pipe(stream)
  });
