const { encryptStream, KmsKeyringNode, needs } = require('@aws-crypto/client-node')
...
const { partition_key, sort_key, ctx_prefix, object_target } = config.document_bucket.document_table
...
const encryptKeyring = new KmsKeyringNode({ generatorKeyId: faytheCMK, keyIds: [walterCMK] })
...
async function store(fileStream, encryptionContext = {}) {
  ...
  needs(!encryptionContext[partition_key] && !encryptionContext[sort_key],
  ...
  const Body = fileStream.pipe(encryptStream(encryptKeyring, { encryptionContext }))

  const file = await s3.upload({ Bucket, Key, Body, Metadata: encryptionContext }).promise()
