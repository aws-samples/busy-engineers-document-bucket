...
export async function createGrant(KeyId: string, GranteePrincipal: string) {
  ...
  if (grants.length > 1)
    throw new Error(
      `Multiple grants exists(${grants.length}). Please delete grants to reset state.`
    );
  if (grants.length === 1) return grants[0];

  return await getRegionalKMS(KeyId)
    .createGrant({
      KeyId,
      GranteePrincipal,
...
export async function existingGrants(KeyId: string, currentPrinciple: string) {
  const { Grants } = await getRegionalKMS(KeyId)
    .listGrants({
      KeyId
    })
    .promise();
  ...
  return Grants.filter(
    ({ GranteePrincipal }) => GranteePrincipal === currentPrinciple
  );
}
